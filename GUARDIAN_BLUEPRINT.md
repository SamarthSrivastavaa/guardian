...
**On-chain write proof — DONE (2026-06-13, testnet, wallet `0x6111…dbaa`).** Against Mysten's LIVE testnet margin (no Guardian package needed), all four steps succeeded on a manager we control (`0x3a209d3a…`):
- create manager — tx `BrCVCXQbpSt1e5YuUWqTgZccXj52PLwDAm9sCoDAYKop`
- deposit 0.3 + borrow 0.15 SUI (Pyth-refreshed PTB) — tx `7WssiTcY7aD2vXQdf7f2RbML1t5wTLcjvVdjpRYNNpLb`
- **repay 0.05 SUI** — tx `2c1yvhHe3WU2fdYzGSxvVp5uxpBBH58FEmAP6U3UaoBn` (debt 0.15→0.10, RR 3.0→4.0, reader-confirmed)
- **cancel_all_orders** — tx `93WMERKRbUtVf8bLTPftLwnZX7ZqtJb57nVAdMUhaEKD` (owner-gated path executes; no-op on zero orders)

**Learned (now Known) — SDK/deployment version drift:** the live testnet margin package is the **older `0xd6a4…` deployment** the SDK's `MARGIN_PACKAGE_ID` points to, NOT the newer vendored `main`. `deposit`/`borrow`/`repay`/`cancel_all_orders` share signatures across versions and work; **`pool_proxy::place_limit_order_v2` does NOT exist on the live deployment** (predates the `_v2` rename) — order placement via the SDK v2 builders aborts `No function … place_limit_order_v2`. The reduce-only-order ladder step therefore needs either the v1 entry on testnet or the self-published stack on localnet. Repay-from-idle (the core deleverage) works against live testnet today.
...
## Phase D — Move contracts (2026-06-13) — build + guard tests DONE, deploy pending gas

**Built & verified:** `contracts/` package `guardian` with three modules, **compiling against the real margin API** (deepbook, deepbook_margin, Pyth, Wormhole, token):
- `guardian::policy` — `ProtectionPolicy` owned object; `create` binds it to `manager.owner()`/`id()`/`deepbook_pool()`; `update`/`revoke` owner-only; `assert_thresholds` enforces the ladder (1.0 < wk < trigger < target) + safety envelope (slippage ≤ 200bps hard cap, tranche bounds); segregated `keeper_tip: Balance<SUI>` (S4).
- `guardian::executor` — `execute_protection` runs ladder steps 1–2 (cancel_all_orders → repay_*(none)) with the **reduce-only postcondition invariant** (`assert_reduce_only`: debt never increases + progress made); `whiteknight_rescue` composes `margin_manager::liquidate` and forwards 100% of seized collateral to the owner. Guards extracted into pure `assert_execution_allowed`/`assert_reduce_only` (single source of truth).
- `guardian::registry` — shared `GuardianRegistry` stats + `GuardianVault` (Bag-of-balances white-knight float, mirroring Mysten's `liquidation_vault`).

**Tests:** `sui move test` → **18/18 pass**, covering S1 (rate-limit, trigger-not-met), S5 (fake policy↔manager binding), S6 (reduce-only: debt-increase + no-progress), S7 (policy bounds: tier, ladder, slippage, tranche) as negative tests each aborting on a specific named code. S2 (oracle staleness) / S3 (MEV slippage) / S9 (white-knight gate) are inherited from the protocol's own checks (safe-oracle `risk_ratio`, reduce-only `max_slippage`, `can_liquidate`) and validated at deploy-time integration (Phase E); S8 (keeper liveness) is architectural.

**Correction to §8 (important):** white-knight cannot fire *above* the pool liquidation threshold — `margin_manager::liquidate` asserts `can_liquidate` (rr < 1.10), so Guardian races to be the liquidator the instant it is legal (capturing the reward for the user) rather than acting earlier. `policy.whiteknight_rr` is a config floor; the binding gate is the protocol's `can_liquidate`. §8.2 sketch updated in spirit.

**Learned / Known (toolchain):** building a third-party package on the margin stack pulls Pyth + Wormhole, whose Sui packages ship `Move.<flavor>.toml` with a placeholder `Move.toml` (literally the text `Move.mainnet.toml`) — `sui move build` fails to parse it until the real flavor manifest is staged over it (one-time cache fix, scripted). `sui move test` also compiles **dependencies' `#[test_only]` code**, and deepbook_margin's test helpers call `pyth::price_info::new_price_info_object_for_test`, present only in the testnet Pyth rev; the `[dep-replacements.testnet]` rev switch did not take effect in sui 1.73 from a dependency manifest, so the helper was added to the cached mainnet Pyth source (documented local-dev patch, orthogonal to Guardian logic).

**Testnet publish attempt (2026-06-13) — DEFERRED TO LOCALNET (by design, §0.5/§0.6).** With gas in hand, `sui client publish` got past dep resolution (after staging **testnet**-flavor Pyth/Wormhole manifests + pinning Pyth's Wormhole dep to the cached building rev) and submitted, but failed on-chain with `VMVerificationOrDeserializationError`: the vendored deepbookv3 `main` source does not byte-match Mysten's deployed testnet margin (the `0xd6a4` version, confirmed in the Phase B write proof), so a Guardian package linked against vendored `main` cannot verify against the live deployment. This is exactly why the executor + crash demo were scoped to **localnet with the full self-published stack** — there, `guardian` links against the exact local deployment with no drift. The package compiles + unit-tests green (18/18); the localnet publish + executor rescue loop is the next on-chain milestone.
