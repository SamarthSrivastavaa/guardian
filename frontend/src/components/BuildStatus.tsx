...
const ROWS: { component: string; status: Status; notes: string }[] = [
  ...
  { component: 'Autopilot — non-custodial pre-signed envelopes', status: 'Live',
    notes: 'execute_protection is owner-gated, so the owner signs the execute-only bytes once (gas pinned, no baked VAA) and the keeper refreshes Pyth just-in-time and relays. Sender stays the owner → validate_owner passes; the keeper holds no authority. Proven on testnet (tx 831h4dc…). Permissionless white-knight covers the at-liquidation case.' },
